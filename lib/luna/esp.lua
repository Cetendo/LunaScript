...
local showDistance, showWanted, showRank, showLanguage, showName, showTags, showHealth, showArmor, showKD, showMoney, showWeapon, showInMyVehicle, showVehicle, showSpeed, hideInterior, showBounty =
        true, true, true, true, true, true, false, false, false, false, true, true, true, false, false, false	
    ...
    for _, pid in players.list(false) do
        ...
        if vehicle then
            ...
            elseif driver == myPed then
                if not showInMyVehicle or VEHICLE.GET_VEHICLE_NUMBER_OF_PASSENGERS(vehicle, false, false) == 0 then
                    goto continue
        ...
        if vehicle and not isMyVehicle then
            posToUse = ENTITY.GET_ENTITY_COORDS(vehicle)     --todo commend that out       
        end
        ...
        elseif vehicle and isMyVehicle then   
            --todo and make good logic here
            
        end
        if showDistance  and math.floor(dist) ~= 0 then
            valuesToDisplay[#valuesToDisplay + 1] = math.floor(dist)
        end
        local nameLine = getName(pid)
        if nameLine:len() > 0 then
            valuesToDisplay[#valuesToDisplay + 1] = nameLine --todo here to
        end
...
local maxDistSlider = menu.slider(espMenu, "Maximum distance", {"espdist"}, "", 5, 10000, maxDistance, 5, function(val)
    maxDistance = val
end)
